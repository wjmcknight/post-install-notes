...
```console
sudo apt install bluez-firmware firmware-linux tuned android-sdk-platform-tools-common htop nmap tmux memtest86+ plocate zsh neovim curl git aptitude build-essential pipx bat fzf fastfetch alacritty galculator gvfs-backends mint-y-icons dmz-cursor-theme
```
...
```console
sudo apt install thunderbird transmission-gtk
```
