# Void Linux Post-Install Notes: KDE
...
```console
sudo xbps-install linux-lts linux-firmware apparmor android-tools android-udev-rules xtools base-devel xorg kde-plasma kde-baseapps spectacle kcalc kwrite NetworkManager avahi cronie cups cups-browsed cups-filters nss-mdns system-config-printer system-config-printer-udev print-manager htop ncurses-term nano nmap tmux memtest86+ plocate zsh neovim haveged uptimed git python3-pipx alacritty bat fzf fastfetch
```
...
```console
sudo xbps-install gwenview exiftool dcraw
```

### Internet

```console
sudo xbps-install chromium chromium-widevine firefox-esr filezilla ktorrent
```

### Multimedia

```console
sudo xbps-install audacity dragon-player kamoso elisa beets python3-Flask ffmpeg flac mpg123 mpg123-pulseaudio mpv eyeD3 yt-dlp
```

### Office

```console
sudo xbps-install calligra-sheets calligra-words okular
```
