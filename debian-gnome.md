...
```console
sudo apt install firmware-linux-nonfree firmware-realtek intel-microcode htop nmap tmux memtest86+ plocate gamin zsh vim vim-gtk3 tuned haveged uptimed bzr git build-essential aptitude python3-pip ruby-rubygems hugo argyll icc-profiles faba-icon-theme moka-icon-theme
```
...
```console
sudo apt install chromium filezilla geary polari
```

### Multimedia

```console
sudo apt install audacity brasero handbrake-gtk sound-juicer beets ffmpeg flac lame mpg123 mpv normalize-audio eyed3 youtube-dl gstreamer1.0-vaapi cmus
```
